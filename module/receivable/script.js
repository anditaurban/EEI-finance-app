...
async function handlePayment(receivableId, projectName, nominal) {
    ...
    // Helper functions untuk handle format Indonesia (titik ribuan)
    const finance = (n) => {
    ...
    const unfinance = (s) => {
        if (!s) return 0;
        const cleanStr = s.toString().replace(/[^0-9]/g, '');
    ...
    const formHtml = `
        <form id="swal-payment-form" class="text-left text-sm text-gray-700">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-3">
                    <div>
                        <label class="block font-semibold mb-1 text-xs text-gray-500 uppercase">Bank Penerima / Kas</label>
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        <label class="block font-semibold mb-1 text-xs text-gray-500 uppercase">Jenis PPh</label>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    <label class="block font-semibold mb-1 text-gray-400 text-[10px] uppercase">Total Invoice (Inc. Tax)</label>
                    <input type="text" id="total_inv_tax_display" class="w-full px-3 py-2 bg-gray-100 border rounded font-medium" readonly>
                </div>
                <div>
                    <label class="block font-semibold text-green-600 mb-1 text-[10px] uppercase text-right">Total Diterima (Net)</label>
                    <input type="text" id="total_receipt_display" class="w-full px-3 py-2 bg-green-50 border border-green-200 text-green-700 rounded font-bold text-xl text-right" readonly>
                </div>
            </div>

            <div class="space-y-3">
                <div><label class="block font-semibold mb-1 text-xs text-gray-500 uppercase">Keterangan</label><textarea id="description" class="w-full px-3 py-2 border rounded" rows="2"></textarea></div>
                <div>
    ...
    const result = await Swal.fire({
        title: `<div class="text-center text-xl font-bold">Input Pembayaran</div>`,
        html: `<div class="text-center mb-4 text-sm text-gray-500 border-b pb-2">Project: <span class="font-semibold text-blue-600">${projectName}</span></div>${formHtml}`,
        width: '850px',
        showCancelButton: true,
        ...
        didOpen: () => {
            ...
            window.calculateTotals = () => {
                ...
                document.getElementById('total_receipt_display').value = finance(totalNet);
            };

            window.updateReceiptNumber = async (date) => {
                try {
            ...
            const initVal = unfinance(nominal);
            ...
            updateReceiptNumber(document.getElementById('receipt_date').value);
            autoSetRate('IDR');
        },
        preConfirm: async () => {
            // Bersihkan format titik ribuan sebelum dikirim ke server
            ...
            // Fix: Nominal dikirim sesuai nilai dasar (bukan 0)
            ...
            formData.append('total_receipt', clean('total_receipt_display'));
