...
function getDataPayload() {
  ...
  const payload = {
    ...
    pph_coa: pphEnabled ? getSelectedPphCoaId() : 0,
...
async function autoSetRate(input) {
  ...
  symbolSpan.textContent = currencySymbols[val] || '';
...
async function loadDetail(Id, Detail) {
  ...
  try {
    ...
    let amount = detail.total_inv || detail.project_amount || 0;
