...
function loadDropdownCall() {
  ...
  <tr class="flex flex-col sm:table-row border rounded sm:rounded-none mb-4 sm:mb-0 shadow-sm sm:shadow-none transition hover:bg-gray-50">  
    ...
    <td class="px-6 py-4 text-sm text-gray-700 border-b sm:border-0 flex justify-between sm:table-cell">
      <span class="font-medium sm:hidden">Email</span>
      ${item.keterangan} ${item.project_name}
    </td>
    ...
    <td class="px-6 py-4 text-sm text-gray-700 text-right w-[15%] flex justify-between sm:table-cell">
      <span class="font-medium sm:hidden">Role</span>
      
    <span class=" ${
      item.transaction_type === 'expenses' || item.transaction_type === 'D'
        ? 'text-red-600'
        : 'text-gray-700'
    }">
      ${item.transaction_type === 'expenses' || item.transaction_type === 'D'
        ? `(D) ${finance(item.nominal)}`
        : finance(item.nominal)}
    </span>
      <div class="dropdown-menu hidden fixed w-48 bg-white border rounded shadow z-50 text-sm">
       <button onclick="event.stopPropagation(); handleEdit(${item.keuangan_id}, '')" class="block w-full text-left px-4 py-2 hover:bg-gray-100">✏️ Edit User</button>
        <button onclick="event.stopPropagation(); handleDelete(${item.keuangan_id})" class="block w-full text-left px-4 py-2 hover:bg-gray-100 text-red-600">
          🗑 Delete User
        </button>
      </div>
    </td>
